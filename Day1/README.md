...
## I/O
- I/O is synchronous
- Inputs are received through the `left` signal for direction (1=left, 0 = right) and the `data_in` signal with default size 10 bits (assuming <1000 clicks per input, although this could be reconfigured for larger) on `data_valid`.
